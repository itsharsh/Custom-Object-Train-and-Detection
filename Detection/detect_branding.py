...
configPath = path_config.detectionModelConfigPath
classesPath = path_config.detectionModelClassesPath
weightsPath = path_config.detectionModelWeightsPath

miscInfo = {
    "channelName": "testvid",
    "videoName": "20200117-130014.mp4",
    "adType": "Branding",
    "videoFPS": 25,
...
#    videoWrite = cv2.VideoWriter(os.path.join(processedVideoDir, videoName),
 #                                cv2.VideoWriter_fourcc('m', 'p', '4', 'v'), miscInfo["videoFPS"], (W, H))
    ...
    while True:
        ...
        cv2.putText(frame, now.strftime("%Y-%m-%d %H:%M:%S.%f"), (10, 30),
                    cv2.FONT_HERSHEY_COMPLEX, 0.75, (255, 255, 255), 1)
